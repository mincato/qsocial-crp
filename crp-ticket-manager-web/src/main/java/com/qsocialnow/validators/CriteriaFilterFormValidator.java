...
public class CriteriaFilterFormValidator extends AbstractValidator {
    ...
    private static final String FILTERADMUNIT_FIELD_ID_PATTERN = "filteradmunit_{0}";
        ...
        if (!CollectionUtils.isEmpty(filter.getFilterWords())) {
            for (int i = 0; i < filter.getFilterWords().size(); i++) {
                ...
                if (StringUtils.isBlank(wordFilter.getInputText())) {
                    addInvalidMessage(context, MessageFormat.format(FILTERWORD_TEXT_FIELD_ID_PATTERN, i),
                            Labels.getLabel(FIELD_EMPTY_LABEL_KEY));
                }
                if (wordFilter.getType() == null) {
                    addInvalidMessage(context, MessageFormat.format(FILTERWORD_TYPE_FIELD_ID_PATTERN, i),
                            Labels.getLabel(FIELD_EMPTY_LABEL_KEY));
                }
        ...
        if (!CollectionUtils.isEmpty(filter.getFilterCategories())) {
            for (int i = 0; i < filter.getFilterCategories().size(); i++) {
                ...
                if (category.getCategoryGroup() == null) {
                    addInvalidMessage(context, MessageFormat.format(FILTERCATEGORY_FIELD_ID_PATTERN, i),
                            Labels.getLabel(FIELD_EMPTY_LABEL_KEY));
                }

            }
        }
        if (!CollectionUtils.isEmpty(filter.getAdmUnitFilters())) {
            for (int i = 0; i < filter.getAdmUnitFilters().size(); i++) {
                AdmUnitFilter adminUnit = filter.getAdmUnitFilters().get(i);
                if (adminUnit.getAdminUnit() == null) {
                    addInvalidMessage(context, MessageFormat.format(FILTERADMUNIT_FIELD_ID_PATTERN, i),
                            Labels.getLabel(FIELD_EMPTY_LABEL_KEY));
                }
